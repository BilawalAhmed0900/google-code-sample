...
class VideoPlayer {
 private:
  ...
  /*
    This will be a pointer inside VideoLibrary, so
    if VideoLibrary changes when this is set,
    puff!!!!!!!!!!!

    As this assignment doesn't really changes anything
    inside VideoLibrary, we are safe for now.

    Should I use two elements instead of this?
    A Video instance and a bool of telling whether
    this is set or not
  */
  const Video *mCurrentVideo = nullptr;
  bool mVideoPlaying = false;

  std::vector<VideoPlaylist> mPlaylists;
